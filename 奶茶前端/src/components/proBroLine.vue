<template>
  <div id="proBroLine" :style="{width:'100%',height:'100%'}"></div>
</template>
<script>
...
　　}  
  ...
  props:["proMonth"],
   data() {
    return {
      color:['#E0EEE0','#036BC8','#696969','#8B7355','#528B8B','#8B6914',"#828282"],
      userBroLine:{
        title: {
          text: "产品销售额折线图",
          textStyle: {
  ...
  methods: {
    initChart() {
      let myChart =  echarts.init(document.getElementById("proBroLine"));
      myChart.setOption(this.userBroLine);
    },
    initData() {
        // console.log(this.proMonth)
        const newMonth= this.proMonth
